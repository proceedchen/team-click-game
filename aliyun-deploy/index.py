...
# 阿里云 Redis 配置
REDIS_HOST = 'r-2zel3534ftcyddbf5i-pub.redis.rds.aliyuncs.com'
REDIS_PORT = 6379
REDIS_PASSWORD = 'Team_click_game'

def execute_redis_command(command):
    """执行 Redis 命令"""
    conn = http.client.HTTPConnection(REDIS_HOST, REDIS_PORT, timeout=10)
    try:
        # 构建 Redis 协议命令
        cmd_parts = command if isinstance(command, list) else [command]
        cmd_str = '*' + str(len(cmd_parts)) + '\r\n'
        for part in cmd_parts:
            part_str = str(part)
            cmd_str += '$' + str(len(part_str)) + '\r\n' + part_str + '\r\n'

        conn.request('GET', '/?' + cmd_str)
...
def get_leaderboard():
    ...
    try:
        # 使用 ZREVRANGE 获取前10
        result = redis_command('ZREVRANGE', 'leaderboard', '0', '9', 'WITHSCORES')
        ...
        for idx in range(0, len(items), 2):
            if idx + 1 < len(items):
                nickname = items[idx]
                score = items[idx + 1]
                if nickname and score and nickname != '':
                    try:
                        leaderboard.append({'nickname': nickname, 'score': int(score)})
                    except:
                        pass
        return leaderboard
...
def submit_score(score, nickname):
    """提交分数"""
    try:
        player_name = nickname.strip() if nickname and nickname.strip() else '匿名玩家'

        # 获取当前分数
        current = redis_command('ZSCORE', 'leaderboard', player_name)
        current_score = 0
        if current and '\r\n' in current:
        ...
        if score > current_score:
            redis_command('ZADD', 'leaderboard', str(score), player_name)

        return {'success': True, 'score': max(score, current_score)}
    except Exception as e:
...
def handler(environ, start_response):
    ...
    if path == '/api/leaderboard' and method == 'GET':
        ...
        response_body = json.dumps({'leaderboard': leaderboard})
        start_response('200 OK', [('Content-Type', 'application/json'), *cors_headers])
        return [response_body.encode()]

    # API: 提交分数
    if path == '/api/submit-score' and method == 'POST':
        try:
            ...
            result = submit_score(score, nickname)
